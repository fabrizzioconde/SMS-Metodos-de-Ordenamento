...
packages <- c("scrime","e1071","kernlab","randomForest", 
              "doParallel", "GA", "ggplot2","care","dplyr")
...
#Função para o cálculo do valor p bruto e ajustado pela corre??o de Bonferroni
...
#svr_test<-list()
mean_svr_ISCORE_list<-list() #Cria lista para a média do SVR sobre o rank da RF
GA<-list()
minimo<-list()
corte<-list()
...
I_scores <- calculate_all_I_scores(dados[[1]], response_var) # Pensar depois em como criar a mesma estrutura da RF para o CARSCORE.
...
#Erro MSE do SVR em relação ao rank CARSCORE

#Inicialização de parâmetros do SVR
gamma = 0.01
cost = 1.0
epsilon = 0.1
folds = 10
...
# Etapa de Corte com o SVR sobre o Rank da RF
mean_svr_ISCORE_list[[i]] <- vector()
passo <- 10

limite <- floor((length(names(dados[[1]][, -dim(dados[[1]])[2]])) / passo) * percentual_snps)

# Constrói a sequência crescente de SNPs mais importantes a partir do rank do CARSCORE
for (cont in 1:(limite + 1)) {
  ...
  if (j == 1) {
    var_sel <- rank_I_scores[1]  # Aqui pegamos o nome da coluna
  } else {
    var_sel <- rank_I_scores[1:j]  # Aqui pegamos os nomes das colunas
  }
  
  # Avalia os 10-fold para o SVR na sequência crescente de SNPs a partir do rank do RF
  svr_test <- validacao_cruzada(data = dados[[1]][, c(var_sel, "fenotipo")],
                                ...
                                epsilon = epsilon, 
                                kernel = kernel)
  mean_svr_ISCORE_list[[i]][cont] <- svr_test[1]
} 


#Constroi o Gráfico do MSE do SVR sobre o CARSCORE
pdf(file="Grafico_MSE_SVR_linear.pdf",height=5,width=9)
plot(seq(passo,limite*(passo)+10,by=passo),
     mean_svr_ISCORE_list[[i]],
     type="o",
     lwd=2,
...
minimo[[i]]<-which.min(mean_svr_ISCORE_list[[i]])
...
snp_selec_corte<-names(rank_I_scores[1:corte[[i]]])
snp_selec_corte

#Selecionando somente o genótipo após o corte
genotipo[[2]]<-dados[[1]][,rank_I_scores[1:corte[[i]]]]
...
maxiter = 10        #Número máximo de gerações do GA.
...
Estatisticas <- c(rep("Mediana", length(geracao)), rep("Média", length(geracao)),rep("Melhor", length(geracao)) )
...
#Etapa de Corte com o SVR sobre o Rank do CARSCORE
mean_svr_ISCORE_list[[i]]<-vector()
passo<-10;
...
for (cont in 1:(limite+1))
{
  ...
  if (j==1) {
    ...
    var_sel<-rank_I_scores[1]
    
  }else{
    
    var_sel<-rank_I_scores[1:j]
...
maxiter = 10    #N?mero m?ximo de gera??es do GA.
pcross = 0.8        #Probabilidade de crossing over
