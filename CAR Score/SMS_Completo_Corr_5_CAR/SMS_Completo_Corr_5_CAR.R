...
#svr_test<-list()
mean_svr_CAR_list<-list() #Cria lista para a média do SVR sobre o rank da RF
GA<-list()
minimo<-list()
corte<-list()
...
car = carscore(dados$x, dados$y) # Pensar depois em como criar a mesma estrutura da RF para o CARSCORE.
...
for (cont in 1:(limite+1))
{
  ...
  svr_test<- validacao_cruzada(data = dados[[1]][c(var_sel,"fenotipo")],
                               ...
                               epsilon = epsilon, 
                               kernel = kernel)
  mean_svr_CAR_list[[i]][cont]<-svr_test[1]
} 
...
snp_selec_corte<-names(rank_car[1:corte[[i]]])
snp_selec_corte

#Selecionando somente o genótipo após o corte
genotipo[[2]]<-dados[[1]][,rank_car[1:corte[[i]]]]
...
#ESCOLHE O PRIMEIRO PONTO DE CORTE
minimo[[i]]<-which.min(mean_svr_CAR_list[[i]][1]) ######## Tive que escolher manualmente o segundo menor MSE, pois o menor valor foi a base de dados com os 100 SNPs.
corte[[i]]<-(minimo[[i]]+1)*passo #Adotou-se o segundo menor MSE do SVR, pois o menor foi o referente ao primeiro marcador SNP2
abline(v=corte[[i]],col="black",lty=2)
dev.off()
...
#ESCOLHE O PRIMEIRO PONTO DE CORTE
minimo[[i]]<-which.min(mean_svr_CAR_list[[i]][1])
corte[[i]]<-(minimo[[i]]+1)*passo #Adotou-se o segundo menor MSE do SVR, pois o menor foi o referente ao primeiro marcador SNP2
abline(v=corte[[i]],col="black",lty=2)
dev.off()
