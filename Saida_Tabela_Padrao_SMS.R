...
process_snps <- function(snps_causais, snps_selecionados) {
  ...
  # Concatenando os SNPs em uma string separada por vírgulas
...
latex_content <- paste0(latex_content, "\\textbf{Método} & \\textbf{$\\gamma$} & \\textbf{\\begin{tabular}[c]{@{}c@{}}\\emph{SNPs}\\\\selecionado\\end{tabular}} & \\textbf{\\# \\emph{SNPs} (V)} & \\textbf{\\begin{tabular}[c]{@{}c@{}}Correlação\\\\média\\end{tabular}} \\\\\n\\hline\n")
...
add_to_table("SNPs causais", "-",result_1$snp_list_str, paste(length(snps_causais), "(", result_1$contagem_presentes, ")", sep=""), "-")
...
writeLines(latex_content, "snps_output.tex")

# Imprimindo mensagem de confirmação
cat("Arquivo 'snps_output.tex' foi gerado com sucesso.\n")
