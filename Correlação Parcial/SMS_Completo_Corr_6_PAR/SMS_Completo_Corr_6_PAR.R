...
for (cont in 1:(limite+1))
{
  ...
  if (j==1) {
    ...
    var_sel<-rank_pcorr[1]
    
  }else{
    
    var_sel<-rank_pcorr[1:j]
...
corte[[i]] <- ifelse((minimo[[i]]+1)*passo > limite, limite, (minimo[[i]]+1)*passo)
...
snp_selec_corte<-names(rank_pcorr[1:corte[[i]]])
snp_selec_corte

#Selecionando somente o genótipo após o corte
genotipo[[2]]<-dados[[1]][,rank_pcorr[1:corte[[i]]]]
