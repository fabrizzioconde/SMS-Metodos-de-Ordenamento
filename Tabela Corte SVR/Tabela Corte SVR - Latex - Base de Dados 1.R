...
df <- data.frame(
  ...
  `PAR Score` = c(80, 90, 90, 30, 40),
  `I Score` = c(30, 30, 20, 20, 20)
)

# Adicionar título
title <- "Número de SNPs selecionados pela etapa de Corte para simulação 1."
...
print.xtable(latex_table, file = "numero_de_snps_selecionados_simulacao1.tex", include.rownames = FALSE, sanitize.text.function = function(x){x})
